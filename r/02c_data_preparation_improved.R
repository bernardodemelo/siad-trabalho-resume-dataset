########################################################
# === 2C. Data Preparation MELHORADO: Features Compostas ===
########################################################
# ESTRATÉGIA: 
# Limpar variáveis inuteis
# Tratar listas
# Limpar novamente
# Criar features compostas entre variáveis relacionadas
# para preservar MÁXIMO de informação antes de tratar valores vazios

source("r/dependencies.R")

cat("========================================================\n")
cat("  DATA PREPARATION MELHORADO v2 - Features Compostas      \n")
cat("========================================================\n\n")
...
# ==================================================#
# FASE 1: SELECÇÃO DE VARIÁVEIS - Remover variaveis #
# ==================================================#

### Remover variáveis com % de dados em falta superior a 75%, com excepção de certication_skill ### 

# Calcular % de missing por coluna
missing_simple <- df %>%
  summarise(across(
    everything(),
    ~ mean(is.na(.) | as.character(.) == "") * 100
  )) %>%
  pivot_longer(cols = everything(),
               names_to = "Variavel",
               values_to = "Pct_Vazios") %>%
  arrange(desc(Pct_Vazios))

missing_simple

# Identificar variáveis a manter (<=75% vazios ou exceção)
vars_to_keep <- missing_simple %>%
  filter(Pct_Vazios <= 75 | Variavel == "certification_skills") %>%
  pull(Variavel)

vars_to_keep

# Criar novo DF filtrado
df_clean <- df %>% select(all_of(vars_to_keep))

cat("Dimensões sem +75%:", nrow(df_clean), "linhas x", ncol(df_clean), "colunas\n\n")


### Remover variáveis irrelevantes para análise ###
#variáveis sem conteudo útil 
irr_vars <- c("company_urls", "extra_curricular_organization_links", "locations")

# Remover
df_clean <- df_clean %>% select(-all_of(irr_vars))

cat("Dimensões sem irrelevantes:", nrow(df_clean), "linhas x", ncol(df_clean), "colunas\n\n")


# ====================================== #
# FASE 2: Transformar Strings em listas  #
# ====================================== #

#####   Listas de variaveis para se tratar #####
## "cols_com_listas": lista de variaveis com listas dentro de Strings
## "cols_com_text": lista de variaveis com texto corrido
## "cols_com_num": lista de variaveis com strings com valor numerico
#####


## cria df com as colunas que contêm listas em string
cols_com_listas <- df_clean %>%
  summarise(across(everything(), ~ any(grepl("\\[", as.character(.))))) %>%
  pivot_longer(cols = everything(),
               names_to = "Variavel",
               values_to = "Tem_Lista") %>%
  filter(Tem_Lista == TRUE) %>%
  pull(Variavel)

##

## cria df com as colunas que contem variaveis com texto corrido
# career_objective, se esta der trabalho acho que podemos descartar, não sei como utilizar!! 

cols_com_text <- c("career_objective", "skills_required","educational_requirements","responsibilities", 
                   "responsibilities.1", "skills_required", "career_objective")

##

## cria df com as colunas que contem variaveis com strings com valor numerico

cols_com_num <- c("experiencere_requirement")

##

#                                  #
#                                  #
# 2.1 Strings que contem uma lista #
# df_clean >> df_clean_list        #
#                                  #
#                                  #

# Limpeza profunda + Tratamento de NA + Conversão para Lista
limpar_e_converter_lista <- function(x) {
  
  # 1. Verifica se a célula já é NA
  if (is.na(x)) return(NA)
  
  # Converte para texto para garantir
  texto <- as.character(x)
  
  # 2. Se for uma string vazia ou apenas "[]", devolve logo NA
  if (texto == "" || texto == "[]") return(NA)
  
  # 3. Limpeza Geral: Remove parenteses, aspas e outros caracteres Python
  # Isto funciona tanto para "['A', 'B']" como para "Google" (fica só "Google")
  texto_limpo <- str_remove_all(texto, "\\[|\\]|'|\"")
  
  # 4. Separação por vírgula
  items <- str_split(texto_limpo, ",")[[1]]
  
  # 5. Tratamento de cada item
  items_finais <- items %>%
    # str_squish remove espaços no início, fim E espaços duplos no meio
    # Resolve o problema do "Student Athlete           "
    str_squish() %>%
    # Filtra valores inválidos, placeholders e strings vazias
    .[! . %in% c("", "None", "N/A", "nan", "null", "NA")] %>%
    # Remove duplicados
    unique()
  
  # 6. Decisão Final: Se a lista ficou vazia, devolve NA. Senão, devolve a List.
  if (length(items_finais) == 0) {
    return(NA)
  } else {
    # as.list garante que o resultado é list("A", "B") e não c("A", "B")
    return(as.list(items_finais))
  }
}

# Aplica a função a todas as colunas que definiste na lista 'cols_com_listas'
df_clean_list <- df_clean %>%
  mutate(across(all_of(cols_com_listas), ~ map(., limpar_e_converter_lista)))
 

#                                         #
#                                         #
# 2.2 Strings que Contem um texto corrido #
#                                         #
#  "career_objective", se der trabalho    # 
#  acho que podemos descartar, não sei    #
#  como utilizar!!                        #
#                                         #
# input >> outupt                         #
#                                         #
...
#                                         #
#                                         #
# 2.3 Strings que Contem valor numérico   #
#                                         #
#                                         #
# input >> outupt                         #
#                                         #
